...
/**
 * Manages players inventory and equipment. All modification and querying of an inventory
 * and equipment should usually come through this class. It is responsible for querying
 * and updating the server on inventory changes.
 */
public class InventoryAndEquipment {
    ...
    /**
     * A single item that is equipped. Encapsulates the item itself and the actual slot it
     * is in.
     */
    ...
    /**
     * A single normal item in the inventory. Encapsults the item itself, the quantity the
     * slot should hold and whether the item can be split or not.
     */
    public class InventoryItem {
        ...
        public String toString() {
            return "InventoryItem{" + "item=" + item + ", quantity=" + quantity + ", canSplit=" + canSplit + ", slot=" + slot + '}';
        }
    ...
    public void rebuild() throws NetworkException {
        ...
        if (player != null) {
            ...
            for (Slot slot : Slot.values()) {
                Item eqIt = player.getEquipment().containsKey(slot) ? getItem(player.getEquipment().get(slot)) : null;
                if (eqIt == null) {
            ...
            while (i < newBagSlots) {
                ...
                if (slot == null) {
                    ...
                    while (i < newBagSlots && amount > 0) {
                        // TODO how do IvMax1 and IvMax2 and IvType1 and IvType2
                        int itemInSlot = Math.max(amount, maxPerSlot);
                        if (LOG.isLoggable(Level.FINE)) {
                            LOG.fine(String.format("Adding %d of %s to slot %d (max %d) leaving %d", itemInSlot, item.getDisplayName(), i, maxPerSlot, amount));
                        }
